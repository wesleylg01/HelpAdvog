...
begin
application.MessageBox ('ERRO 002 - Já exite um usuário com o CPF informado, revise os dados e tente novamente', 'ERRO - 002' , MB_ICONWARNING);
Edit_nome.SetFocus;
 end
 else
 salvar;
...
begin
    //----------- TESTANDO SE NÃO TEM NENHUM CAMPO EM BRANCO -----------------//
    if ((Edit_nome.Text = '')or (Edit_tel.text = '')or (Edit_cel.Text = '')
       ...
       then
    begin
        ShowMessage('ERRO 001 - Atenção Preencha todos os campos para completar o operação');
    end;
    //------------------------------------------------------------------------//

    //---- CONFERINDO SE O CAMPO SENHA E O CAMPA CONFIRMA SENHA SÃO IGUAIS ---//

    if (Edit_senha.Text <> Edit_confirmasenha.Text) then
      begin
            ShowMessage('ERRO 001 - As senhas digitadas não conferem, tente novamente');
            Edit_senha.Clear;
            Edit_confirmasenha.Clear;
            Edit_senha.SetFocus;
      end;
